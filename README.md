...
A browser port of the Tkinter desktop app (`../argos_modbus_builder.py`). Same
4-step wizard (Import → Map → Edit → Preview), now fronted by an output-format
picker on the Import step. Each format is a self-contained "variant bundle"
...
1:1 from the Python reference and locked to it byte-for-byte by parity tests.
The same XML-import path (skip straight to the editor) is preserved for any
variant that declares a `parse` step.
...
assert the TS port matches the Python oracle byte-for-byte. The fixtures are
generated by running the real Python code:

```bash
# from the repo root, in the Python dev image:
python3 web/scripts/gen_fixtures.py
```

If you change `../argos_modbus_builder.py`, regenerate the fixtures and re-run the
tests to keep the web port in sync.
